...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["product_image"]["name"])) {
        ...
        if (move_uploaded_file($_FILES["product_image"]["tmp_name"], $target_file)) {
            ...
            $stmt = $conn->prepare("INSERT INTO womendress (product_name, product_price, product_image) VALUES (?, ?, ?)");
            if ($stmt === false) {
            ...
            if ($stmt->execute()) {
                echo "<script>alert('Product added successfully!'); window.location.href='womendresss.php';</script>";
            } else {
